...
def fit_circle_2d(p1, p2, p3):
    ...
    R = np.sqrt((x1-center[0])**2 + (z1-center[1])**2)
...
def visualize(U_hist, coords, tip_hist, tip_v_hist, tip_a_hist):
    ...
    # 取最终形状
    U_final = U_hist[-1].reshape(-1,6)[:,:3]
    coords_def = coords + U_final

    # 取三点（首、中、尾）
    p1 = coords_def[0]
    p2 = coords_def[len(coords_def)//2]
    p3 = coords_def[-1]

    center, R_fit = fit_circle_2d(p1, p2, p3)
    ...
    print("\n========== 圆弧对比 ==========")
    print("理论半径 R_theory =", R_theory)
    print("拟合半径 R_fit   =", R_fit)
    print("相对误差 =", abs(R_fit - R_theory)/R_theory)
    print("================================\n")

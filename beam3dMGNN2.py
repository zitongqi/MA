...
def append_pred_to_window(
    ...
    """
    X_window:
        [1, SEQ_LEN, N, 17] normalized input window

    pred_y_norm:
        [1, N, 12] normalized model output in y-space
    ...
    # 只替换 u,v
    # --------------------------------------------------------
    last_x = X_window[:, -1, :, :].clone()  # [1, N, 17]

    # --------------------------------------------------------
    # pred_y_norm 是 y-space normalized
    # 先反归一化成真实 u,v
    # --------------------------------------------------------
    pred_uv_real = normalizer.denorm_y(
        pred_y_norm.squeeze(0)
    )  # [N, 12]

    # --------------------------------------------------------
    # 固定节点强制 u,v = 0
    # fixed flag 在 x 的最后一维
    # --------------------------------------------------------
    fixed_mask = last_x[0, :, -1] > 0.5

    pred_uv_real[fixed_mask, :] = 0.0

    # --------------------------------------------------------
    # 把真实 u,v 转换回 x-space normalized
    # x 的 u,v 在 3:15
    # --------------------------------------------------------
    x_mean = normalizer.x_mean.to(pred_uv_real.device)
    x_std = normalizer.x_std.to(pred_uv_real.device)

    pred_uv_x_norm = (
        pred_uv_real - x_mean[3:15]
    ) / x_std[3:15]

    # --------------------------------------------------------
    # 更新 last_x 中的 u,v
    # --------------------------------------------------------
    last_x[:, :, 3:15] = pred_uv_x_norm.unsqueeze(0)

    # --------------------------------------------------------
    # 滑动窗口：
    # 去掉最早一步，加上预测的新一步
    # --------------------------------------------------------
    new_X_window = torch.cat(
        [
            X_window[:, 1:, :, :],
            last_x.unsqueeze(1)
        ],
...
# ============================================================
# visualize free rollout response
# ============================================================
def visualize_rollout_response(
        model,
        dataset,
        normalizer,
        edge_index,
    ...
    if len(selected_indices) <= rollout_steps:

        rollout_steps = len(selected_indices) - 1

    start = random.randint(
        0,
        len(selected_indices) - rollout_steps - 1
    )

    rollout_indices = selected_indices[
        start:start + rollout_steps
    ]

    # ========================================================
    # 初始窗口：只用第一个真实窗口
    # ========================================================
    first_idx = rollout_indices[0][0]

    X0, Y0, _, _ = dataset[first_idx]

    X_window = X0.unsqueeze(0).to(device)  # [1, SEQ_LEN, N, 17]

    num_nodes = Y0.shape[0]
    ...
    with torch.no_grad():
        ...
        for step, (idx, _) in enumerate(rollout_indices):

            # ------------------------------------------------
            # 当前真实 target
            # ------------------------------------------------
            _, Y_gt, _, _ = dataset[idx]
            ...
            # 下一步不再使用真实 FEM 输入
            # ------------------------------------------------
            X_window = append_pred_to_window(
                X_window,
                pred_y_norm,
                normalizer
            )
    ...
    for row, n in enumerate(selected_nodes):
        ...
        axes[row, 0].set_title(
            f"Node {n} - rollout u{['x','y','z'][direction]}(t)"
        )

        axes[row, 0].set_xlabel("Rollout step")
        axes[row, 0].set_ylabel("Displacement")
        axes[row, 0].legend()
        ...
        axes[row, 1].set_title(
            f"Node {n} - rollout v{['x','y','z'][direction]}(t)"
        )

        axes[row, 1].set_xlabel("Rollout step")
        axes[row, 1].set_ylabel("Velocity")
        axes[row, 1].legend()
...
def main():
    ...
    # )
    # ========================================================
    # free rollout visualization
    # ========================================================
    visualize_rollout_response(
        model,
        test_dataset,
        normalizer,
        edge_index,
